...
import { Text, View, StyleSheet, FlatList, Pressable,KeyboardAvoidingView, Platform,InputAccessoryView,Alert } from "react-native";
...
const ComentaryView = ({id,Comentarys, updateRoute}) => {
	...
	const WriteCommentary = () => {
		...
		axios.post(url, body, headers)
			.then(() => {
				axios.get(eco_explore_api + `/bitacoras/${id}`).then(res => {
					// console.log(res.data);
					updateRoute(res.data);
					axios.post(eco_explore_api + "/bitacoras/comentarios", {"Comentarios": res.data.Comentarios}).then((result) => {
						setIsLoading(false);
		...
		<SafeAreaView style={styles.container}>
			...
			<>
				{loading? 
					...
					:
					<>
						{!error && !comentarios.length? <EmptyCarrousell title="Ocurrio un problema al recuperar los comentarios"/> : 
									...
									{token.access_token? 
										<View style={styles.calificationArea}>
											...
											<KeyboardView  style={styles.insertArea}>
												<InputIcon placeholder="Ingresa Un Comentario." value={nuevoComentario} onTextChange={(value) => setNuevo(value)} iconName={"arrow-up-circle"} color={"green"} onPress={() => {
													console.log("Presed");
													WriteCommentary();
												}}/>
											</KeyboardView>
