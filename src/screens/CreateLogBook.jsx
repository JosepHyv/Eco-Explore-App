...
const Formulario = ({setId, setError}) => {
	...
	const validas = ["caminata", "montañismo", "ciclismo","escalada","rapel","cañonismo"];
	...
	const validar = () => {
		...
		{
			Alert.alert("Las unicas actividades permitidas son Caminata,Ciclismo,Montañismo,Escalada,Rapel,Cañonismo ");
			return;
		}
		if(!difVa.includes(dificultad.toLocaleLowerCase()))
		{
			Alert.alert("Las unicas dificultades permitidas son Facíl, Moderado y Difícil");
			return;
		}
	...
	return (
		<View style={{flex:1, justifyContent:"center", alignItems:"center"}}>
			<Text>Nombre</Text>
			<InputLabel placeholder="Nombre" value={nombre} onChangeText={(value) => setN(value)}/>
			<Text>Descripcion</Text>
			<InputLabel placeholder="Descripcion" value={descripcion} onChangeText={(value) => setDes(value)}/>
			<Text>Actividad</Text>
			<InputLabel placeholder="Actividad" value={actividad} onChangeText={(value) => setAct(value)}/>
			<Text>Actividad</Text>
			<InputLabel placeholder="Dificultad" value={dificultad} onChangeText={(value) => setDif(value)}/>
...
const Bitacora = ({setVisible, visible}) => {
	...
	useEffect(() => {
		(async () => {
			...
			setLocation({"Lon":location.coords.latitude, "Lat": location.coords.longitude});
		})();
	});
	return (
		<>
...
const CreateLogBook = ({navigation}) => {
	...
	useEffect(() => {
		if(token !== undefined && token.access_token)
		{
			...
			axios.get(urlRutas, headers).then(response => {
			...
			axios.get(urlExplo, headers).then(response => {
				setLoadAgenda(false);
							...
							<>
								...
								<View style={{flex:1, gap:10}}>
									<View style={{alignItems:"center", }}>
										...
										<Text style={{fontSize:30, fontWeight:"500", marginVertical:15}}>{usuario.Nombre + " " + usuario.ApellidoPaterno }</Text>
										<View style={styles.horizontalContainer}>
											<View style={{flexDirection:"column", justifyContent:"center", alignItems:"center"}}>
												<Text style={{fontSize:20, fontWeight:"500"}}>{publicas}</Text>
												<Text style={{fontSize:20, fontWeight:"500"}}>Bitacoras</Text>
											</View>
											<View style={{flexDirection:"column", justifyContent:"center", alignItems:"center"}}>
												<Text style={{fontSize:20, fontWeight:"500"}}>{guardadas}</Text>
												<Text style={{fontSize:20, fontWeight:"500"}}>Guardadas</Text>
											</View>
											<View style={{flexDirection:"column", justifyContent:"center", alignItems:"center"}}>
												<Text style={{fontSize:20, fontWeight:"500"}}>{agendadas}</Text>
												<Text style={{fontSize:20, fontWeight:"500"}}>Agendadas</Text>
											</View>
										</View>
									</View>
									<View style={{flex:1, alignItems:"center", gap:10, marginHorizontal:20}}>
										<View style={{height:200, width:"100%",  borderRadius:18, backgroundColor:"#68A044", alignItems:"center"}}>
											{/* <Text></Text> */}
											<Image 
												source={require("../../assets/mapas.png")}
												style={{height:120, width:120}}
											/>
											<Text style={{fontSize:20, fontWeight:"600", color:"white"}}> Creatu Bitacora</Text>
											<Text style={{fontSize:20, fontWeight:"600", color:"white", textAlign:"center"}}>Utiliza el mapa, traza tus rutas y cuenta tus historias</Text>

										</View>
										<EcoButton title="Crear Bitacora" onPress={() => {setVisible(!visible);}}/>
									</View>
								</View>
								<Modal
									visible={visible} 
